...
{
	double SF_error = 0.;
	const double effData = get_EfficiencyData(pt, eta);
	const double effMC = get_EfficiencyMC(pt, eta);
	const double errData = get_EfficiencyDataError(pt, eta);
	const double errMC =  get_EfficiencyMCError(pt, eta);

	if(effData != 0 && effMC != 0)
		SF_error = std::hypot(errData/effData, errMC/effMC) * (effData / effMC);
	return SF_error;
}
